...
def elimination(document):
    ...
    for b in tagged:
        if (not (b[1] == "." or b[1] == "IN" or b[1] == "DT" or b[1] == "PRP$" or b[1] == "(" or b[1] == ")" or b[
            1] == "," or b[1] == "VBZ" or b[1] == "PRP" or b[1] == "VBD" or b[1] == "CC" or b[1] == "RB" or b[
                     1] == "RBS")):
            data.append(b[0])
        ...
        for row in read:
            if (i != 0):
                if (j < size and document[j] - 1 == i):
                    output.append([row[0], row[1], row[2], ast.literal_eval(row[3]), area[i - 1], keywords[i - 1],
                                   abstract[i - 1]])
                    j += 1
            i += 1
